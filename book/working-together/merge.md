...
`hg rename` [^id2] command to rename it, so that Mercurial can do the right thing later when we merge.

[^id2]: If you're a Unix user, you'll be glad to know that the `hg rename` command can be abbreviated as `hg mv`.
...
the same name. This is documented as [issue 29](https://bz.mercurial-scm.org/show_bug.cgi?id=29).
